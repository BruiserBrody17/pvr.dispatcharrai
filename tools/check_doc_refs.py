...
Usage: python tools/check_doc_refs.py
        ...
        for lineno, line in enumerate(doc.read_text(encoding="utf-8").splitlines(), start=1):
            ...
            for m in SECTION_TITLE_RE.finditer(line):
                ...
                if not any(normalized_title in h or h in normalized_title for h in headings):
                    errors.append(
                        f'{doc.relative_to(REPO_ROOT)}:{lineno}: cites "{title}" section '
                        f"in {target_path.relative_to(REPO_ROOT)}, no matching heading found"
                    )
    return errors
        ...
        for lineno, line in enumerate(doc.read_text(encoding="utf-8").splitlines(), start=1):
            for m in FUNCTION_REF_RE.finditer(line):
                ...
                if not re.search(rf"\b{re.escape(name)}\b", src_text):
                    errors.append(
                        f"{doc.relative_to(REPO_ROOT)}:{lineno}: references `{name}()`, "
                        "not found in src/ or either plugin's plugin.py -- verify it isn't "
                        "an external (Kodi-core/library/stdlib) citation before treating as stale"
                    )
    return errors
        ...
        for lineno, line in enumerate(doc.read_text(encoding="utf-8").splitlines(), start=1):
            ...
            for m in SETTING_REF_RE.finditer(line):
                ...
                if token not in known:
                    errors.append(
                        f"{doc.relative_to(REPO_ROOT)}:{lineno}: references setting `{token}`, not found in "
...
def main() -> int:
    errors = check_section_titles() + check_functions() + check_settings()
    if not errors:
        print("check_doc_refs: no dangling references found")
        return 0
    print(f"check_doc_refs: {len(errors)} possibly-dangling reference(s) -- verify each by hand:\n")
    for error in errors:
        print(error)
    return 1
